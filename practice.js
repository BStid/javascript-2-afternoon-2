...
function evenFinder(nums){
  const newArr = nums.filter((value,index,array) => {
    return value % 2 === 0;
  })
  return newArr;
}
...
const divider = function(numbersArray){
  evensArray = [];
  oddsArray = [];
...
function finder(arr){
  const rando = getRandomArbitrary();
...
addTen = numbers =>  numbers.map(function(value){
